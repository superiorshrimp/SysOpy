...
int main(int argc, char* args[]){
    ...
    if(argc<2){printf("%s\n", "too few arguments! exiting..."); return 1;}
    void* handle = dlopen("./libmemory_blocks.so", RTLD_LAZY);
    if(!handle){return 1;}
    ...
    create_memory_block_array = dlsym(handle, "create_memory_block_array");
    delete_memory_block_array = dlsym(handle, "delete_memory_block_array");
    create_memory_block = dlsym(handle, "create_memory_block");
    remove_memory_block = dlsym(handle, "remove_memory_block");
    //find_free_index = dlsym(handle, "find_free_index");
    add_memory_block = dlsym(handle, "add_memory_block");
    lc_command = dlsym(handle, "lc_command");
    wc_command = dlsym(handle, "wc_command");
    cc_command = dlsym(handle, "cc_command");
    read_and_copy = dlsym(handle, "read_and_copy");
    save_results = dlsym(handle, "save_results");

    FILE* f0 = fopen("./results_of_counting_if_enabled.txt", "w");
    fclose(f0);
